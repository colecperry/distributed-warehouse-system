...
# Credit Card - Scale on CPU (70%)
resource "aws_appautoscaling_policy" "credit_card_cpu" {
  name               = "${var.project_name}-credit-card-cpu-scaling"
  ...
  target_tracking_scaling_policy_configuration {
    ...
    target_value       = var.credit_card_cpu_target
    scale_in_cooldown  = var.scale_in_cooldown
    scale_out_cooldown = var.scale_out_cooldown
  }
}

# Warehouse - Scale on Memory (75%)
...
# Shopping Cart - Scale on Memory (75%)
...
output "auto_scaling_summary" {
  ...
  value = {
    ...
    credit_card   = "CPU @ ${var.credit_card_cpu_target}%"
    warehouse     = "Memory @ ${var.warehouse_memory_target}%"
    product       = "CPU @ ${var.product_cpu_target}%"
